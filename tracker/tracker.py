...
from SuperGluePretrainedNetwork.models.utils import frame2tensor, read_image
...
class Tracker():
    ...
    def camera_motion_computation(self, prev_img, curr_img):
        ...
        if self.features == "superglue":
            matching = Matching(config).eval().to('cuda')
            prev_tensors = frame2tensor(prev_img, device='cuda')
            curr_tensors = frame2tensor(curr_img, device='cuda')
            pred = matching({'image0':prev_tensors, 'image1':curr_tensors})
            
            keypoints0 = pred['keypoints0'][0].cpu().numpy()
            keypoints1 = pred['keypoints1'][0].cpu().numpy()
            matches = pred['matches0'].cpu().numpy().flatten()

            valid = matches > -1
            prev_pts = keypoints0[valid]
            curr_pts = keypoints1[matches[valid]]
